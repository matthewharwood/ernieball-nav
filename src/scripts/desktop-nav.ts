...
export class DesktopNav {
  public static run() {
    // Set up Selectors
    const rootSiteLink = document.querySelector(
      `[${DataAttrs.ROOT}]`
    ) as HTMLElement;
    const overlay = document.querySelector(
      `.${ClassNames.DESKTOP_NAV}`
    ) as HTMLElement;
    const links = document.querySelectorAll(
      `[${DataAttrs.LINK}]`
    ) as NodeListOf<HTMLElement>;
    const outlets = document.querySelectorAll(
      `[${DataAttrs.OUTLET}]`
    ) as NodeListOf<HTMLElement>;

    // Set up DataStructure
    const mapLinksOutlets = (
      ls: NodeListOf<HTMLElement>,
      os: NodeListOf<HTMLElement>
    ): Map<string, Map<string, HTMLElement>> => {
      const map = new Map();
      Array.from(ls).forEach(l => {
        Array.from(os).forEach(o => {
          if (
            o.attributes.getNamedItem(DataAttrs.OUTLET).value ===
            l.attributes.getNamedItem(DataAttrs.LINK).value
          ) {
            const val = l.attributes.getNamedItem(DataAttrs.LINK).value;
            map.set(
              val,
              new Map().set(DataAttrs.OUTLET, o).set(DataAttrs.LINK, l)
            );
          }
        });
      });
    ...
    const setActiveLink = (id = '*') => {
      ...
      linksOutletsMap.forEach(l =>
        l.get(DataAttrs.LINK).classList.remove(ClassNames.ACTIVE)
      );
    ...
    const activate = t => {
      ...
      linksOutletsMap.forEach(v =>
        v.get(DataAttrs.OUTLET).classList.remove(ClassNames.ACTIVE)
      );
      linksOutletsMap
        .get(linkId)
        .get(DataAttrs.OUTLET)
        .classList.add(ClassNames.ACTIVE);
